...
applesearch.init = function ()
{
	...
	if ( navigator.userAgent.toLowerCase().indexOf('safari') < 0  && document.getElementById )
	{
		...
		if (dummy)	dummy.href = "../view/searchBox/applesearch.css";
	}
}
...
applesearch.onChange = function (fldID, btnID)
{
	...
	{
		btn.style.background = "white url('../view/searchBox/srch_r_f2.gif') no-repeat top left";
		btn.fldID = fldID; // btn remembers it's field
		btn.onclick = this.clearBtnClick;
		this.clearBtn = true;
	} else if (fld.value.length == 0 && this.clearBtn)
	{
		btn.style.background = "white url('../view/searchBox/srch_r.gif') no-repeat top left";
		btn.onclick = null;
		this.clearBtn = false;
	}
}
